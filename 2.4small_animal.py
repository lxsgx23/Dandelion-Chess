...
# 棋盘常量调整
ROWS, COLS = 9, 7
TILE_SIZE = 66  # 根据棋盘图片和屏幕分辨率调整
ANALYSIS_PANEL_WIDTH = 500  # 分析面板宽度保持不变
WIDTH, HEIGHT = COLS * TILE_SIZE + ANALYSIS_PANEL_WIDTH, 768  # 总高度设为768
...
# GTP控制台常量
GTP_CONSOLE_HEIGHT = 174 
GTP_MAX_LENGTH = 100
GTP_FONT_SIZE = 16
FONT_SCALE = 0.8
SCROLL_SPEED = 3
# 提示栏常量
INFORMATION_PANEL_POS = 50  # 信息面板位置调整
INFORMATION_PANEL_HEIGHT = 200
...
class XiangQi:
    ...
    def __init__(self):
        ...
        self.board_img = pygame.image.load("pieces/board.jpg").convert()
         # 调整棋盘图片缩放
        self.board_img = pygame.transform.scale(pygame.image.load("pieces/board.jpg").convert(), 
    ...
    def draw_analysis_panel(self):
        ...
        panel_height = HEIGHT - GTP_CONSOLE_HEIGHT  # 使用剩余高度
        pygame.draw.rect(self.screen, (240, 240, 240), (panel_x, 0, ANALYSIS_PANEL_WIDTH, panel_height))
        ...
        y += 30
        with self.analysis_lock:
            for idx, result in enumerate(self.analysis_results[:8]):  # 显示数量减少
                text_line = f"{idx+1}. {result['move']}: {result['winrate']:.1f}%"
    ...
    def draw_information_panel(self):
        ...
        x0 = COLS*TILE_SIZE
        panel_height = HEIGHT - GTP_CONSOLE_HEIGHT - y0 - 20  # 计算可用高度
        pygame.draw.rect(self.screen, (160, 160, 160), 
                        (x0, y0, ANALYSIS_PANEL_WIDTH, panel_height))
        
        font = pygame.font.SysFont(FONT_NAME, 16)  # 字体缩小
        y = y0 + 10
        line_spacing = 22  # 行间距缩小
